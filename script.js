        ...
        trainSelectorOptions += systemTrains
            .map(([id, train]) => {
                ...
                <button
                    ...
                    onpointerdown="train470LongPressStart(event)"
                    onpointerup="train470LongPressEnd(event, true)"
                    onpointerleave="train470LongPressEnd(event, false)"
                    oncontextmenu="return false;"
                >
                    ${train.name}
                    <span class="train-470-units-hint">Mantén para unidades</span>
                </button>
            `;
                }
                return `
...
// ============================================================
// SISTEMA DE UNIDADES DEL TREN 470
// ============================================================

const TRAIN_470_UNITS_KEY = 'train470Units';

let _470longPressTimer = null;
let _470longPressActivated = false;

function train470LongPressStart(e) {
    e.preventDefault();
    _470longPressActivated = false;
    _470longPressTimer = setTimeout(() => {
        _470longPressActivated = true;
        _470longPressTimer = null;
        const selector = document.getElementById('train-selector');
...
function train470LongPressEnd(e, wasPointerUp) {
    if (_470longPressTimer) {
        clearTimeout(_470longPressTimer);
        _470longPressTimer = null;
        // Fue tap corto: seleccionar tren normalmente
        if (wasPointerUp && !_470longPressActivated) {
            selectTrain('470');
            toggleTrainSelector();
        }
    }
...
function save470Units(units) {
    try {
        localStorage.setItem(TRAIN_470_UNITS_KEY, JSON.stringify(units));
    } catch (e) {
        console.warn('Error al guardar unidades 470', e);
    }
}

function delete470Unit(unitName) {
...
function apply470Unit(unitName) {
    ...
    render();
    close470UnitsModal();
    showToast(`Unidad ${unitName} cargada`);
}

function open470UnitsModal() {
    const existing = document.getElementById('modal-470-units');
    ...
    modal.innerHTML = `
        <div class="units470-modal modal">
            ...
            <div class="units470-search-wrap">
                <input
                    id="units470-search"
                    class="units470-search"
                    type="text"
                    placeholder="Ej: 470.157"
                    oninput="render470UnitsList()"
                    autocomplete="off"
                    autocorrect="off"
                    spellcheck="false"
                />
    ...
    `;
    modal.addEventListener('click', e => {
        if (e.target === modal) close470UnitsModal();
    });
    document.body.appendChild(modal);
    lockBodyScroll();
    render470UnitsList();
    setTimeout(() => document.getElementById('units470-search')?.focus(), 100);
}

function close470UnitsModal() {
    const modal = document.getElementById('modal-470-units');
    if (modal) modal.remove();
    unlockBodyScroll();
}

function render470UnitsList() {
    const list = document.getElementById('units470-list');
    if (!list) return;

    const query = (document.getElementById('units470-search')?.value || '').trim().toLowerCase();
    const units = load470Units();
    const entries = Object.entries(units).filter(([name]) =>
        !query || name.toLowerCase().includes(query)
    );

    if (entries.length === 0) {
        const msg = query
            ? `<div class="units470-empty">No hay unidades que coincidan con "<strong>${escapeHtml(query)}</strong>".<br>Usa el botón de abajo para guardar la configuración actual con ese nombre.</div>`
            : `<div class="units470-empty">No hay unidades guardadas todavía.<br>Configura las variantes y usa el botón de abajo para guardar.</div>`;
        list.innerHTML = msg;
        return;
    }
    ...
    list.innerHTML = entries
        ...
        .map(([name, variants]) => {
            const variantsSummary = Object.entries(variants)
                .map(([c, v]) => `${c}:${v}`)
                .join(' · ');
...
function confirm470UnitDelete(unitName) {
    if (confirm(`¿Eliminar la unidad "${unitName}"?`)) {
        delete470Unit(unitName);
    }
}

function save470CurrentUnit() {
    const input = document.getElementById('units470-search');
    const rawName = (input?.value || '').trim();

    if (!rawName) {
        showToast('Introduce un número de unidad (ej: 470.157)');
        input?.focus();
        return;
    }

    const units = load470Units();
    const variantsCopy = { ...state.coach470Variants };

    if (units[rawName]) {
        if (!confirm(`La unidad "${rawName}" ya existe. ¿Sobreescribir?`)) return;
    }

    units[rawName] = variantsCopy;
    save470Units(units);
    render470UnitsList();
...
function showToast(msg) {
    const existing = document.getElementById('toast-470');
    if (existing) existing.remove();
    const toast = document.createElement('div');
    toast.id = 'toast-470';
    toast.className = 'toast-470';
    toast.textContent = msg;
    document.body.appendChild(toast);
    requestAnimationFrame(() => toast.classList.add('toast-470-visible'));
    setTimeout(() => {
        toast.classList.remove('toast-470-visible');
